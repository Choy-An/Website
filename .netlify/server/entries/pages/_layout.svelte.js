...
const items = [
  ...
  { title: "Other Works", to: "/experience", icon: "i-carbon-development" },
  { title: "About Me", to: "/education", icon: "i-carbon-education" },
...
const NavMenu = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_page();
  $$unsubscribe_theme();
  return `<div class="nav-menu svelte-jdc7ud"><nav class="container flex flex-row items-center text-sm"><a${add_attribute("href", `${base}/`, 0)} class="nav-menu-left decoration-none w-auto md:w-150px lg:w-auto row flex flex-row items-center cursor-pointer px-4 text-[var(--secondary-text)] self-stretch hover:bg-[color:var(--main-hover)]"><img src="/logos/aswanth logo.png" alt="My Logo" class="my-logo w-10 h-auto"> <span class="ml-2 text-md font-bold hidden md:inline overflow-hidden whitespace-nowrap text-ellipsis">${escape(name)} ${escape(lastName)}</span></a> <div class="flex-1 block overflow-hidden md:hidden whitespace-nowrap text-ellipsis text-center">${escape(name)} ${escape(lastName)}</div> <div class="flex-row flex-1 self-center h-full justify-center hidden md:flex">${each(items, (item) => {
